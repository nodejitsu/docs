var findit = require('findit'), 
    ...
    koala = require('koala'),
    uglify = require('uglify-js').parser.parse;
...
article.weld = function(dom, articles) {
  ...
  Object.keys(articles).forEach(function(i){
    ...
    //
    Array.prototype.forEach.call( $('code', dom), function (c) {
      var src = $(c).text(),
          rendered;

      // test to see if uglify thinks the code snippet is javascript
      // Makes too many false negatives, imo.
      try {
        uglify(src);
        // "snippet.js" is recognized by koala as a js filename
        rendered = koala.render('snippet.js', src);        
      } catch (e) {
        //assume it's not javascript. Still have to handle entities though.
        rendered = $("<div>").text(src).html();
      }
